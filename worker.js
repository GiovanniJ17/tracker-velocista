  ...
  async fetch(request, env) {
    ...
    try {
      ...
      const inferredSchema =
        responseSchema || (responseFormat?.type === 'json_object' ? { type: 'object' } : null)
      ...
      console.log(
        '[Worker] Custom key from header:',
        customKey ? `${customKey.substring(0, 10)}...` : 'none'
      )

      // 2. Priorità: custom header > body apiKey > env variable
      const resolvedApiKey = customKey && customKey.length > 10 ? customKey : apiKey
      console.log(
        '[Worker] Resolved API key:',
        resolvedApiKey ? `${resolvedApiKey.substring(0, 10)}...` : 'none'
      )
      ...
      if (provider === 'gemini') {
        ...
        if (!geminiKey) {
          return new Response(
            JSON.stringify({ error: { message: 'Gemini API key non configurata' } }),
            {
              status: 500,
              headers: { ...corsHeaders, 'Content-Type': 'application/json' }
            }
