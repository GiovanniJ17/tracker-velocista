...
function extractPersonalBests(text) {
  ...
  if (text.match(/\bgara\b|\bpista\b|\bcompetizione\b|\bgare\b/i)) {
    const implicitRacePattern =
      /(?:gara|pista|competizione)\s*:?\s*(\d+)\s*m(?:etri?)?\s+(?:in\s+)?(\d+[.,]\d+|\d+)\s*(?:sec|s)?(?!\s+(?:x|serie|set|\d+x))/gi
  ...
  const strengthPattern =
    /(squat|bench|deadlift|stacco|clean|jerk|press|military\s+press|panca|trazioni?)\s+(\d+[.,]\d+|\d+)\s*kg\s+(?:PB|personal\s+best|massimale|nuovo\s+massimale)/gi
  ...
  if (text.match(/\bpalestra\b|\bpeso massimale\b|\bmassimali\b/i)) {
    const implicitStrengthPattern =
      /(?:palestra|forza)\s*:?\s*(squat|bench|deadlift|stacco|clean|jerk|press|military\s+press|panca|trazioni?)\s+(\d+[.,]\d+|\d+)\s*kg(?!\s+(?:x|reps|set))/gi
...
function extractInjuries(text) {
  ...
  const injuryPattern =
    /(infortunio|dolore|lesione|strappo muscolare|contusione|distorsione|tendinite|infiammazione)\s+(?:(?:dietro\s+)?(?:alla\s+|al\s+|di\s+))?([a-z\s]+?)(?:\.|,|;|$|\s+(?:grave|moderato|lieve))/gi
  let match
  ...
  while ((match = injuryPattern.exec(text)) !== null) {
    ...
    bodyPartText = bodyPartText
      .replace(/^dietro\s+|^davanti\s+|^interno\s+|^esterno\s+/i, '')
      .trim()
