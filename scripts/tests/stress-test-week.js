...
if (!supabaseUrl || !supabaseAnonKey) {
  console.error(
    '❌ Variabili di ambiente mancanti. Configura VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY'
  )
  process.exit(1)
}
        ...
        sets: [
          {
            ...
            weight_kg: 100,
            recovery_s: 180,
            details: { intensity: 'max' }
          },
          {
            exercise_name: 'Spalle manubri',
            category: 'lift',
            sets: 3,
            reps: 8,
            weight_kg: 35,
    ...
    groups: [
      {
        ...
        sets: [
          {
            exercise_name: 'Corsa fondo',
            category: 'endurance',
            distance_m: 12000,
            time_s: 3600,
            recovery_s: 0,
            details: { pace: 'z2', elevation: 200 }
          }
        ]
      }
    ]
    ...
    groups: [
      {
        ...
        sets: [
          ...
          {
            exercise_name: 'Drills coordinativi',
            category: 'drill',
            sets: 5,
            reps: 30,
            recovery_s: 20,
    ...
    groups: [
      {
        ...
        sets: [
          {
            exercise_name: 'Stretching gambe',
            category: 'mobility',
            sets: 1,
            time_s: 600,
            recovery_s: 0,
    ...
    groups: [
      {
        ...
        sets: [
          ...
          {
            exercise_name: 'Progressive runs',
            category: 'sprint',
            distance_m: 1000,
            recovery_s: 60,
            details: {}
      ...
      },
      {
        ...
        sets: [
          {
            exercise_name: '5km gara',
            category: 'sprint',
            distance_m: 5000,
            time_s: 1320,
            recovery_s: 0,
            details: { pace: '4:24', heart_rate_max: 195 }
          }
        ]
      }
    ]
    ...
    groups: [
      {
        ...
        sets: [
          {
            exercise_name: 'Corsa lenta 15km',
            category: 'endurance',
            distance_m: 15000,
            time_s: 5400,
            recovery_s: 0,
            details: { pace: 'z1', heart_rate_avg: 130 }
          }
        ]
      }
    ]
...
class StressTestSuite {
  ...
  async testConnection() {
    ...
    try {
      const { data, error } = await supabase.from('training_sessions').select('id').limit(1)
  ...
  async testComplexQueries() {
    ...
    try {
      ...
      if (e1) throw e1
      const avgRPE = (sessions.reduce((sum, s) => sum + (s.rpe || 0), 0) / sessions.length).toFixed(
        1
      )
      ...
      const { data: allSets, error: e3 } = await supabase.from('workout_sets').select('category')
      ...
      const totalDistance = allSets.reduce((sum, s) => sum + (s.distance_m || 0), 0) / 1000
      const totalWeight = allSets.reduce(
        (sum, s) => sum + (s.weight_kg || 0) * (s.sets || 1) * (s.reps || 1),
        0
      )
      log(
        `  Volume settimanale`,
        `${totalDistance.toFixed(1)}km, ${totalWeight.toFixed(0)}kg totali`,
        'verify'
      )
  ...
  async testDataIntegrity() {
    ...
    try {
      ...
      const { data: sets, error: e3 } = await supabase.from('workout_sets').select('group_id')
      ...
      if (integrityOk) {
        log(
          '✓ Integrità',
          `Verificata - ${sessions.length} sessioni, ${groups.length} gruppi, ${sets.length} set`,
          'success'
        )
        this.results.passed++
      } else {
  ...
  async testDeleteAndRestore() {
    ...
    try {
      ...
      log(
        '✓ Eliminazione',
        `Cascata DELETE verificata - ${groupCountBefore} record eliminati`,
        'success'
      )
      this.results.passed++
    } catch (error) {
  ...
  async printResults() {
    ...
    log(
      'SUCCESSO GLOBALE',
      `${percentage}% (${this.results.passed}/${totalTests} test)`,
